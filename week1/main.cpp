...
int main()
{
    ...
    screen_objects[1] = new Ball { sf::Vector2f{ SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 }, sf::Color::Yellow, 15.0, 90, 2 };
    screen_objects[2] = new Wall { sf::Vector2f{ SCREEN_WIDTH / 2, WALL_SIZE / 2 }, SCREEN_WIDTH, WALL_SIZE, sf::Color::Green };  // top wall
    screen_objects[3] = new Wall { sf::Vector2f{ SCREEN_WIDTH - WALL_SIZE / 2, SCREEN_HEIGHT / 2 }, WALL_SIZE, SCREEN_HEIGHT, sf::Color::Green }; // right wall
    screen_objects[4] = new Wall { sf::Vector2f{ SCREEN_WIDTH / 2, SCREEN_HEIGHT - WALL_SIZE / 2 }, SCREEN_WIDTH, WALL_SIZE, sf::Color::Green };
    screen_objects[5] = new Wall { sf::Vector2f{ WALL_SIZE / 2, SCREEN_HEIGHT / 2 }, WALL_SIZE, SCREEN_WIDTH, sf::Color::Green }; 

    // screen_objects[2] = new Ball { sf::Vector2f{ SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 }, sf::Color::Blue, 35.0, 225, 2 };
    ...
    while(window.isOpen()) {
        ...
        for (int i = 0; i < AMOUNT_OF_SCREEN_OBJECTS; i++) {
            if (dynamic_cast<Ball*>(screen_objects[i])) {
                dynamic_cast<Ball*>(screen_objects[i]) -> collider.is_colliding(screen_objects[0]);
            }
